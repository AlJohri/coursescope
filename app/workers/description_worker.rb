class DescriptionWorker
  ...
  def get_json(type, *paths)
    case type
      when /^(terms)$/;
        page = @agent.get(BASE1 + EXT1[1..-1])
      when /^(schools|subjects)$/;
        page = @agent.get(BASE1 + paths.join('/') + EXT1)
      when /^(courses|sections)$/;
        page = @agent.get(BASE2 + paths.join('/') + EXT1)
      when /^(data)$/;
        page = @agent.get(BASE2 + paths.join('/') + EXT2)
    end
